...
/**
 * A simple helper module for checking menu items against the current module
 * (controller action) (as far as the Router knows).
 *
 * @param string    $item       The name of the module to check against. Can be an array of names.
 ...
 */
function check_module($item, $class_only = false)
{
     if (strtolower(get_instance()->router->module) == strtolower($item)) {
        return $class_only ? 'active' : 'active';
    }
