...
if (! function_exists('_form_common')) {
    ...
    function _form_common($type = 'text', $data = '', $value = '', $label = '', $extra = '', $tooltip = '')
    {
        ...
        if (function_exists('form_error') && form_error($defaults['name'])) {
            $error   = ' error';
            $tooltip = '<span class="help-inline text-danger">' . form_error($defaults['name']) . '</span>';
        }
...
<div class='form-group row{$error}'>
    <label class='col-4 col-form-label' for='{$defaults['id']}'>{$label}</label>
    <div class='col-8'>
         <input class='form-control{$is_invalid}' {$output} {$extra} />
        {$tooltip}
    </div>
</div>";
    }
...
if (! function_exists('form_dropdown')) {
    ...
    function form_dropdown($data, $options = array(), $selected = array(), $label = '', $extra = '', $tooltip = '')
    {
        ...
        return "
<div class='form-group row{$error}'>
    <label class='col-4 col-form-label' for='{$data['id']}'>{$label}</label>
    <div class='col-8'>
        <select class=\"custom-select\" {$output} {$extra}>
            {$options_vals}
        </select>
        {$tooltip}
    </div>
