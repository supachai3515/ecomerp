...
if ($db_required != '') {
    ...
    if ($usePagination) {
        ...
        \$this->pager['total_rows']  = \$this->{$module_name_lower}_model->count_all();
    ...
    if($useSoftDeletes){
        $indexFind .= "
            \$records = \$this->{$module_name_lower}_model->find_all_by(array('deleted' => false));";
    }else{
        $indexFind .= "
...
if ($db_required != '') {
    ...
    if($useSoftDeletes){
        $editFind = "
            \${$module_name_lower} = \$this->{$module_name_lower}_model->find_by(array('id' => \$id, 'deleted' => false));
            if(!\${$module_name_lower}){ 
                Template::set_message(lang('{$module_name_lower}_invalid_id'), 'danger');
                redirect(SITE_AREA . '/content/{$module_name_lower}'); 
            }
            Template::set('{$module_name_lower}', \${$module_name_lower}); ";
    }else{

        $editFind = "
            \${$module_name_lower} = \$this->{$module_name_lower}_model->find(\$id);
            if(!\${$module_name_lower}){ 
                Template::set_message(lang('{$module_name_lower}_invalid_id'), 'danger');
                redirect(SITE_AREA . '/content/{$module_name_lower}'); 
            }
