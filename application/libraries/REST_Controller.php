...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        $this->rest->company_id = NULL;
        $this->rest->ignore_limits = FALSE;

        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            if ( ! ($row = $this->rest->db->where(array($this->config->item('rest_key_column') => $key, 'deleted' => false))->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
            ...
            isset($row->company_id) && $this->rest->company_id = $row->company_id;
    ...
    protected function _perform_library_auth($username = '', $password = NULL)
    {
        ...
        $application_name = $this->_get_application_name();
        ...
        return $this->{$auth_library_class}->$auth_library_function($username, $password, $application_name);
    }

    /**
     * Check application name requested
     * By Monthon Nunlaong
     * 
     * @access protected
     * @return bool $application_name
     */
    protected function _get_application_name()
    {

        $application_name_available = 'HTTP_' . strtoupper(str_replace('-', '_', 'application'));
        $application_name = $this->input->server($application_name_available) ?? 'Unknown';
        
        $application_name = empty($application_name) ? 'Unknown' : $application_name;
