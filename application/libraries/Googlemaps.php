...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyBWnDAgyZ1PT9TvMLm08E7D40pSbFvDh-Y'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $center						= "13.7244416,100.3529095";		// Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"
	...
	var $zoom						= 6;						// The default zoom level of the map. If set to "auto" will autozoom/center to fit in all visible markers. If "auto", also overrides the $center parameter
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		if ($this->geocodeCaching) { // if caching of geocode requests is activated
			...
			$CI =& get_instance();
			$google_db = $CI->load->database('google', TRUE); 
			$google_db->select("latitude,longitude");
			$google_db->from("geocoding");
			$google_db->where("address", trim(strtolower($address)));
			$query = $google_db->get();
		...
		//$data_location .= "maps.google.com/maps/api/geocode/json?address=".$address."&sensor=".$this->sensor;
		...
		if ($data->status=="OK") 
		{
			...
			if ($this->geocodeCaching) { // if we to need to cache this result
				if ($address != "" && $lat != 0 && $lng != 0)
				{
					...
					$google_db->insert("geocoding", $data);
				}
			}
		}
		else
	...
	function get_coordinates($lat1, $lon1, $lat2, $lon2)
	{
	    
	   	$url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=km&language=th&origins=" . $lat1 ."," . $lon1 . "&destinations=" . $lat2 . "," . $lon2 . "&key=AIzaSyC5WnG1Y1e9HpWE9-HvV70PdpsxgTaQflQ";
	   	//$url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=km&language=th&origins=Washington,DC&destinations=New+York+City,NY&key=AIzaSyC5WnG1Y1e9HpWE9-HvV70PdpsxgTaQflQ";
	    ...
	    //print_array($response_a->rows[0]->elements[0]->distance->text);
	    ...
	    {
	        $return = array('distance' => $response_a->rows[0]->elements[0]->distance->text, 'duration' => $response_a->rows[0]->elements[0]->duration->text);
	        return $return;
	    }
	}
