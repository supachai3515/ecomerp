...
class App_hooks
{
    ...
    public function __construct()
    {
        ...
        $this->ignored_controllers = config_item('hooks_ignored_controllers');
    ...
    public function checkSiteStatus()
    {
        ...
        if (
            !$this->ci->auth->has_permission('Site.Signin.Offline')
    ...
    public function prepRedirect()
    {
        if (!empty($this->ignored_controllers)) {
            ...
            foreach ($this->ignored_controllers as $key => $value) {
                $exp = explode("/", $value);
                if ($exp[1] == "*") {
                    $this->ignored_controllers[$key] =  "{$module}/{$controller}";
                }
            }
            ...
            if (in_array($path, $this->ignored_controllers)) {
                log_message('info', "Hooks prepRedirect skipped for '{$path}'");
                return $this;
            }
        }
