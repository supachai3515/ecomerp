 ...
 * Content controller
 */
class new_car extends Admin_Controller
{
    ...
    public function index()
    {
        try {
            //set title
            Template::set('toolbar_title', lang('new_car_title'));
            Assets::add_module_js('new_car', 'new_car.js');
    ...
    public function search()
    {
        try {
            ...
            //Total number
            $sql = "SELECT count(*) total   FROM sellingnewcar u WHERE 1=1 $sqlwhere";
            $result = $this->db->query($sql);
            $totalRecords = $result->row()->total;
            $sql = "SELECT u.* FROM  sellingnewcar u  WHERE 1=1 $sqlwhere ORDER BY  $columnName  $columnSortOrder  limit  $row , $rowperpage ";
            $result = $this->db->query($sql);
    ...
    public function view()
    {
        try {
            ...
            $data['action']  = "view";
            $data['new_car_data']  = $this->new_car_model->get_sellingnewcar_byid($id);
            $this->load->view("new_car_partial", $data);
        } catch (\Throwable $th) {
            $this->output
