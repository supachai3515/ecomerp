...
<ul class="nav nav-tabs mt-4">
	<li class="nav-item"><?php echo anchor($index_url, lang('us_tab_all'), 'class="nav-link'.(($filter_type == 'all') ? ' active' : '').'"'); ?></li>
    <li class="nav-item"<?php echo $filter_type == 'inactive' ? ' class="active"' : ''; ?>><?php echo anchor("{$index_url}inactive/", lang('us_tab_inactive'), 'class="nav-link'.(($filter_type == 'inactive') ? ' active' : '').'"'); ?></li>
    <li class="nav-item"<?php echo $filter_type == 'banned' ? ' class="active"' : ''; ?>><?php echo anchor("{$index_url}banned/", lang('us_tab_banned'), 'class="nav-link'.(($filter_type == 'banned') ? ' active' : '').'"'); ?></li>
    <li class="nav-item"<?php echo $filter_type == 'deleted' ? ' class="active"' : ''; ?>><?php echo anchor("{$index_url}deleted/", lang('us_tab_deleted'), 'class="nav-link'.(($filter_type == 'deleted') ? ' active' : '').'"'); ?></li>
	<li class="nav-item dropdown">
		<a href="#" class="nav-link <?php echo $filter_type == 'role_id' ? 'active ' : ''; ?> dropdown-toggle" data-toggle="dropdown">
			<?php
		...
		<ul class="dropdown-menu">
            ...
            <li class="dropdown-item"><?php echo anchor("{$index_url}role_id-{$role->role_id}", $role->role_name); ?></li>
			<?php endforeach; ?>
		</ul>
	</li>
</ul>
<?php if (empty($users) || ! is_array($users)) : ?>
<p class="mt-2"><?php echo lang('us_no_users'); ?></p>
<?php
else :
    $numColumns = 8;
    echo form_open();
?>
    <table class="table table-striped user-table mt-1">
		<thead>
			<tr>
