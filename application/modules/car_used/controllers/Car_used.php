 ...
 * Content controller
 */
class car_used extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->load('car_used');
        $this->load->model("car_used_model");
    ...
    public function index()
    {
        try {
            //set title
            Template::set('toolbar_title', lang('car_used_title'));
            Assets::add_module_js('car_used', 'car_used.js');
    ...
    public function search()
    {
        try {
            ...
            //Total number
            $sql = "SELECT count(*) total   FROM sellingcar u WHERE 1=1 $sqlwhere";
            $result = $this->db->query($sql);
            $totalRecords = $result->row()->total;
            $sql = "SELECT u.* FROM  sellingcar u  WHERE 1=1 $sqlwhere ORDER BY  $columnName  $columnSortOrder  limit  $row , $rowperpage ";
            $result = $this->db->query($sql);
    ...
    public function view()
    {
        try {
            ...
            $data['action']  = "view";
            $data['car_used_data']  = $this->car_used_model->get_sellingcar_byid($id);
            $this->load->view("car_used_partial", $data);
        } catch (\Throwable $th) {
            $this->output
